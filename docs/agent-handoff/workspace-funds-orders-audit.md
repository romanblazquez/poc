...
- Removed Angular local cache artifacts from git tracking by adding `.angular/` to `.gitignore` and untracking `.angular/cache/**` via `git rm --cached` so Angular compiler cache files stay local-only.
- Updated detached Dockview behavior to hide the tab strip entirely (`.tabs-and-actions-container`) under `.detached-workspace`, so detached windows show panel content only while normal (non-detached) workspaces keep full tabs and editing affordances.
- Replaced hardcoded workspace presets in the shell with a dynamic workspace tab model persisted in `localStorage` (`fdc3.workspace-tabs.v5`), including an `+` action to add new workspaces at runtime. New tabs are saved with name/id/panel list/state, legacy `v4` state is migrated on first load, and detached workspace recall now restores into dynamic tabs (creating the tab if needed).
- Added close controls on workspace tabs in the shell. Tabs can now be removed directly from the tab bar (while preventing removal of the last remaining tab), and closing the active tab automatically focuses the nearest remaining tab while cleaning up associated workspace state/detached references.
- Fixed Angular app startup/build failures caused by unresolved `./theme-tokens.css` from shared workstation styles: introduced `libs/shared-ui/src/styles/theme-tokens.scss` and switched `workstation-angular.scss` to `@use './theme-tokens.scss'` so Sass resolves tokens within the shared library context. Verified with `npm run build:apps` (all 9 apps) and `npx nx run desktop-shell:build`.
- Fixed inner app visual theme drift (example: Funds Allocations) by correcting invalid AG Grid theme mappings in `libs/fdc3-core/src/types/theme-config.ts`: replaced non-existent `ag-theme-dark-financial` with valid `ag-theme-quartz-dark` for dark/high-contrast themes. This allows AG Grid skin switching to respond to theme changes again.
- Fixed delayed/non-reactive theme updates in inner Angular apps (including Customer Search and Funds Allocations): app root theme listeners now use `window.fdc3.getTheme()` + `window.fdc3.onThemeChanged(...)` and run updates inside `NgZone` with `ChangeDetectorRef.markForCheck()`. This ensures Angular change detection always runs when preload-driven theme events arrive.
- Fixed AG Grid staying light in dark shell themes by updating shared Angular workstation styling selectors in `libs/shared-ui/src/styles/workstation-angular.scss`: removed stale non-existent theme class selectors, added valid dark selectors (`ag-theme-quartz-dark`, `ag-theme-alpine-dark`), and bound AG Grid color variables/root/header/row styling to shared workstation tokens so inner grids now follow shell dark/high-contrast palettes consistently.
- Added explicit dark/high-contrast AG Grid background enforcement in `libs/shared-ui/src/styles/workstation-angular.scss` for core grid internals (`.ag-root-wrapper`, center/body viewports, header containers, pinned containers, rows, odd rows, cells). This prevents any residual white grid surfaces when dark mode tokens are active, even if a light AG Grid skin class is present.
- Improved AG Grid selected-row UX contrast (especially in dark mode): introduced dedicated selection tokens in `libs/shared-ui/src/styles/theme-tokens.scss` (`--ws-selection-bg`, `--ws-selection-text`, `--ws-selection-border`) with per-theme values, and wired `workstation-angular.scss` selected/focus row and selected-cell styles to those tokens so foreground text remains legible and keyboard-focus state has a clear visual outline.
- Resolved remaining dark-mode selected-row contrast regressions caused by per-cell inline color styles (gain/loss and buy/sell columns): updated AG Grid `cellStyle` callbacks in `incoming-orders`, `funds-allocations`, `market-watch`, and `portfolio-view` to switch to `var(--ws-selection-text)` when `node.isSelected()` is true. Added a CSS safety net in `workstation-angular.scss` to force selected-row descendant text color (`.ag-row-selected .ag-cell *`) for nested renderers.
- Updated dark-mode selected-row UX per design direction: in `theme-tokens.scss`, dark/high-contrast selection tokens now use a brighter selected background with a dark foreground text color (`--color-selection-text`) so selected AG Grid rows render dark text on selected state in dark mode.
- Updated shell workspace-tab UX in `apps/desktop-shell/src/renderer/App.tsx`: newly added workspaces now start empty instead of inheriting the active workspace panels, and workspace names can be renamed inline by double-clicking a tab, editing the label, and confirming with Enter or blur (Escape cancels).
- Improved shell/Interop Flow workspace UX in `apps/desktop-shell/src/renderer/App.tsx` and Interop Flow components: the Interop Flow view is now scoped to the active workspace app set instead of the full app directory, the flow hook reloads persisted graphs reactively when the active workspace changes, the React Flow surface follows the selected shell theme via shared token-driven styling, workspace tabs are hidden while `App Launcher` mode is active, and the `Workspace` / `Interop Flow` / `App Launcher` mode switch is presented as a grouped segmented control.
